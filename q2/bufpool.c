...
static char *pageInBuf(BufPool pool, int slot)
{
    char *pname;
...
static int pageInPool(BufPool pool, char rel, int page)
{
    int i;
    char id[MAXID];
...
static int findFree(BufPool pool)
{
    int i;
    int slot = NONE;
    for (i = 0; i < pool->nbufs; i++)
...
static int findVictim(BufPool pool)
{
    int slot = NONE;
